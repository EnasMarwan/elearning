...
<x-dashboard-layout>
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">{{__('Add Lesson')}}</h6>
        </div>
        <div class="card-body">
        <form action="{{ route('lessons.store' , $chapter->id) }}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="row justify-content-center">
                <div class="col">
                    <label for="title" class="control-label">{{__('Name Lesson')}} :</label>
                    <input class="form-control @error('name') is-invalid @enderror " placeholder="Name" name="name" id= "name" type="text">
                    @error('name')
                ...
                <div class="col-12 col-lg-8 form-group">
                    ...
                    <input class="form-control @error('description') is-invalid @enderror " placeholder="Input short description of Lesson" name="description" id= "description" type="text">
                    @error('description')
                    <p class="invalid-feedback"> {{$message}}</p>  
                    @enderror 
                </div>
                ...
                {{-- <div class="col-12 col-lg-8 form-group">
                    <label for="file" class="control-label">{{__('Upload File')}} :</label>
                ...
                {{-- <div class="col-12 col-lg-8 form-group">
                    <label for="video" class="control-label">{{__('Upload video')}} :</label>
                    <input class="form-control"  name="video" id= "video" type="file">
                    @error('video')
                    <p class="invalid-feedback"> {{$message}}</p>  
                    @enderror 
                </div> --}}
